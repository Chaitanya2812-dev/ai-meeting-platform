...
export default function Login() {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", fontFamily: "var(--font-body)" }}>
      ...
      <div style={{
        width: "50%", background: "linear-gradient(135deg,#5b3ff8 0%,#7c3aed 50%,#4f46e5 100%)",
        ...
        {/* Center — headline + video */}
        <div style={{ position:"relative", zIndex:2, flex:1, display:"flex", flexDirection:"column", justifyContent:"center", gap:20, paddingTop:24, paddingBottom:24 }}>

          <div>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(255,255,255,0.15)", backdropFilter:"blur(8px)", borderRadius:100, padding:"6px 16px", marginBottom:14 }}>
              <span style={{ width:8, height:8, borderRadius:"50%", background:"#4ade80", display:"inline-block" }} />
              <span style={{ fontSize:"0.8rem", color:"rgba(255,255,255,0.9)", fontWeight:500 }}>Powered by Groq AI</span>
            </div>
            <h2 style={{ fontFamily:"var(--font-display)", fontSize:"2rem", fontWeight:800, color:"white", lineHeight:1.15, letterSpacing:"-0.04em", marginBottom:10 }}>
              Turn meetings into<br />actionable insights
            </h2>
            <p style={{ fontSize:"0.88rem", color:"rgba(255,255,255,0.72)", lineHeight:1.65, maxWidth:320 }}>
              Upload any audio or video. Get transcripts, summaries, action items, and chat with your meeting instantly.
            </p>
          </div>
          ...
          <div style={{
            ...
            {/* Fake browser chrome */}
            <div style={{ background:"rgba(0,0,0,0.6)", padding:"7px 12px", display:"flex", alignItems:"center", gap:6 }}>
              <span style={{ width:9, height:9, borderRadius:"50%", background:"#ff5f57", display:"inline-block" }} />
              <span style={{ width:9, height:9, borderRadius:"50%", background:"#febc2e", display:"inline-block" }} />
              <span style={{ width:9, height:9, borderRadius:"50%", background:"#28c840", display:"inline-block" }} />
              <span style={{ flex:1, background:"rgba(255,255,255,0.08)", borderRadius:4, padding:"2px 10px", fontSize:"0.68rem", color:"rgba(255,255,255,0.4)", marginLeft:8, fontFamily:"monospace" }}>
                ai-meeting-platform.vercel.app
              </span>
            </div>
            <video
              src="https://res.cloudinary.com/dybuzsqlr/video/upload/v1778214878/ai_meeting_demo_c9hngu.mp4"
              autoPlay
              muted
              loop
              playsInline
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {["🎙️ Transcription","📋 AI Summary","✅ Action Items","💬 Chat","📧 Email","🌍 10+ Languages"].map(f => (
              <span key={f} style={{ background:"rgba(255,255,255,0.12)", backdropFilter:"blur(8px)", border:"1px solid rgba(255,255,255,0.18)", borderRadius:100, padding:"5px 12px", fontSize:"0.73rem", color:"rgba(255,255,255,0.88)", fontWeight:500 }}>{f}</span>
            ))}
          </div>
        </div>

        {/* Footer quote */}
        <p style={{ position:"relative", zIndex:2, fontSize:"0.78rem", color:"rgba(255,255,255,0.4)", fontStyle:"italic" }}>
      ...
      <div style={{
        ...
        <div style={{ width:"100%", maxWidth:400 }} className="anim-up">
          <div style={{ marginBottom:32 }}>
            ...
            <p style={{ color:"var(--text-secondary)", fontSize:"0.9rem" }}>Sign in to your MeetingAI account</p>
          </div>
            ...
            <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:18 }}>
              ...
              <div>
                ...
                <div style={{ position:"relative" }}>
                  <input className="input" type={showPass ? "text" : "password"} required value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" style={{ paddingRight:42 }} />
              ...
              <button type="submit" disabled={loading} className="btn btn-primary" style={{ width:"100%", height:44, fontSize:"0.9rem", marginTop:4, borderRadius:12 }}>
                {loading
                  ? <span style={{ display:"flex", alignItems:"center", gap:8 }}><span className="spinner" />Signing in...</span>
                  : "Sign in"
                }
              </button>
            </form>
        ...
        <div style={{ marginTop:48, textAlign:"center" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:16, justifyContent:"center" }}>
            <a href="https://www.linkedin.com/in/chaitanya-naik-82b44a2b8" target="_blank" rel="noreferrer"
              style={{ display:"flex", alignItems:"center", gap:6, color:"var(--text-secondary)", textDecoration:"none", fontSize:"0.82rem", fontWeight:500 }}
